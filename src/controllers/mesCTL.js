...
export const deleteMessage = async (req, res) => {
  ...
  if (!messageId) {
    return res.status(400).json({ message: "Lỗi lấy ID của tin nhắn" });
  }
  ...
  try {
    ...
    await message.deleteOne(); // Use deleteOne instead of delete (deprecated in some versions)
...
export const toggleMessageReaction = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Không có quyền tương tác với tin nhắn này" });
    }
    ...
    // Gửi thông báo realtime
    getIO()
      .to(`chat_${message.sender}_${message.receiver}`)
