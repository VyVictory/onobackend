...
export const deleteReport = async (req, res) => {
  try {
    ...
    if (!reportId || !type) {
      return res
        .status(400)
        .json({ message: "Thiếu thông tin reportId hoặc type" });
    }
    ...
    switch (type) {
      case "post":
        deletedReport = await PostReport.findByIdAndDelete(reportId);
        break;
      case "user":
        deletedReport = await UserReport.findByIdAndDelete(reportId);
        break;
      case "comment":
        deletedReport = await CommentReport.findByIdAndDelete(reportId);
        break;
      default:
        return res.status(400).json({ message: "Loại báo cáo không hợp lệ" });
    }
    ...
    if (!deletedReport) {
      return res
        .status(404)
        .json({ message: "Báo cáo không tồn tại hoặc đã bị xóa" });
    }

    res
      .status(200)
      .json({ message: "Xóa báo cáo thành công", report: deletedReport });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
