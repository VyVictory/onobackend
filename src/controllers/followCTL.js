...
export const followUser = async (req, res) => {
    try {
        ...
        if (error.code === 11000) {
            return res.status(400).json({ message: 'Đã theo dõi người dùng này trước đó' });
        }
...
export const unfollowUser = async (req, res) => {
    try {
        ...
        res.json({ message: 'Đã hủy theo dõi người dùng' });
    } catch (error) {
        res.status(500).json({ message: 'Lỗi khi hủy theo dõi', error: error.message });
    }
};
