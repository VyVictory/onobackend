...
export const sendFriendRequest = async (req, res) => {
    try {
        ...
        getIO().to(`user_${recipientId}`).emit('notification', {
            type: 'FRIEND_REQUEST',
...
export const respondToFriendRequest = async (req, res) => {
    try {
        ...
        if (status === 'accepted') {
            ...
            // Gửi thông báo realtime
            getIO().to(`user_${friendship.requester._id}`).emit('notification', {
                type: 'FRIEND_ACCEPTED',
                notification: await notification.populate('sender', 'firstName lastName avatar')
            });
        }
...
export const getReceivedFriendRequests = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Lỗi khi lấy danh sách lời mời kết bạn', error });
    }
};
...
export const getSentFriendRequests = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Lỗi khi lấy danh sách lời mời đã gửi', error });
    }
};
